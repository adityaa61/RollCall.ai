    ...
    if photo_source:
        ...
        with st.spinner("AI is scanning..."):
            ...
            if num_faces == 0:
                ...
                if detected:
                    # FIX: the face pipeline returns dict keys as strings
                    # (or numpy types), while `student["student_id"]` from the
                    # database is a plain int. Without casting to int here,
                    # the equality check below (`s["student_id"] == student_id`)
                    # always failed, so recognized students were incorrectly
                    # told "Face not recognized! You might be a new student!"
                    student_id = int(
                        list(detected.keys())[0]
                    )
        ...
        with st.container(border=True):
            ...
            try:
                audio_data = st.audio_input(
                    "Record a short phrase like 'I am present' or 'My name is Akash'."
                )
            except Exception:
                st.error(
